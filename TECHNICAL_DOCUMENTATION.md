# BioSync - Umfassende Technische Dokumentation
## Klärgruben-Monitoring-System

**Version:** 2.0  
**Datum:** November 2025  
**Autor:** LukeArrow, BioSync Development Team  
**Status:** Produktionsreif

---

## Dokumenten-Übersicht

Dieses Dokument bietet eine sehr detaillierte technische Beschreibung des BioSync Klärgruben-Monitoring-Systems. Es richtet sich an Techniker, Entwickler und Wartungspersonal, die das System verstehen, aufbauen, in Betrieb nehmen oder erweitern möchten.

### Zielgruppe

- **Elektronik-Techniker:** Für Hardware-Aufbau und -Wartung
- **Software-Entwickler:** Für Firmware-Anpassungen und Erweiterungen
- **Installations-Techniker:** Für Montage und Inbetriebnahme
- **Wartungspersonal:** Für Wartung, Diagnose und Reparatur

### Verwandte Dokumente

- **[HARDWARE_DETAILED.md](HARDWARE_DETAILED.md)** - Detaillierte Hardware-Dokumentation
- **[SOFTWARE_ARCHITECTURE.md](SOFTWARE_ARCHITECTURE.md)** - Software-Architektur und Code-Dokumentation
- **[SENSOR_INTEGRATION.md](SENSOR_INTEGRATION.md)** - Sensor-spezifische Dokumentation
- **[COMMUNICATION_PROTOCOL.md](COMMUNICATION_PROTOCOL.md)** - RS-485 Protokoll-Spezifikation
- **[NEXTION_HMI_GUIDE.md](NEXTION_HMI_GUIDE.md)** - Nextion HMI Anleitung
- **[TROUBLESHOOTING_GUIDE.md](TROUBLESHOOTING_GUIDE.md)** - Fehlerbehebung
- **[CALIBRATION_PROCEDURES.md](CALIBRATION_PROCEDURES.md)** - Kalibrierungsverfahren
- **[INSTALLATION_DETAILED.md](INSTALLATION_DETAILED.md)** - Installationsanleitung
- **[WIRING_GUIDE.md](WIRING_GUIDE.md)** - Verdrahtungsanleitung

---

## Inhaltsverzeichnis

1. [Systemübersicht](#1-systemübersicht)
2. [Technische Architektur](#2-technische-architektur)
3. [Systemkomponenten](#3-systemkomponenten)
4. [Stromversorgungsarchitektur](#4-stromversorgungsarchitektur)
5. [Kommunikationssystem](#5-kommunikationssystem)
6. [Betriebsmodi und Zustandsmaschinen](#6-betriebsmodi-und-zustandsmaschinen)
7. [Timing und Synchronisation](#7-timing-und-synchronisation)
8. [Fehlerbehandlung und Redundanz](#8-fehlerbehandlung-und-redundanz)
9. [Performance-Charakteristiken](#9-performance-charakteristiken)
10. [Umgebungsbedingungen und Zuverlässigkeit](#10-umgebungsbedingungen-und-zuverlässigkeit)
11. [Erweiterbarkeit und Skalierbarkeit](#11-erweiterbarkeit-und-skalierbarkeit)
12. [Sicherheit und Compliance](#12-sicherheit-und-compliance)
13. [Anhänge](#13-anhänge)

---

## 1. Systemübersicht

### 1.1 Projektbeschreibung

BioSync ist ein verteiltes Echtzeit-Monitoring-System zur kontinuierlichen Überwachung von Klärgruben und Abwasser-Systemen. Das System erfasst kritische Parameter wie Füllstand, Temperatur, Trübung und TDS-Wert (Total Dissolved Solids) und visualisiert diese auf einem benutzerfreundlichen Touchdisplay.

#### Hauptziele

- **Zuverlässigkeit:** 99,9% Verfügbarkeit im Dauerbetrieb
- **Wartungsarmut:** Minimaler Wartungsaufwand durch robuste Komponenten
- **Benutzerfreundlichkeit:** Intuitive Bedienung über Touchdisplay
- **Erweiterbarkeit:** Modularer Aufbau für zukünftige Erweiterungen
- **Kosteneffizienz:** Einsatz kostengünstiger Standard-Komponenten

### 1.2 Systemarchitektur - Überblick

Das BioSync-System besteht aus zwei Hauptknoten (Nodes), die über eine RS-485 Verbindung kommunizieren:

```
╔══════════════════════════════════════════════════════════════════════════╗
║                        BIOSYNC SYSTEM ARCHITEKTUR                        ║
╚══════════════════════════════════════════════════════════════════════════╝

    🏠 DISPLAY NODE (Haus)              📡 RS-485              🌊 SENSOR NODE (Schacht)
    ┌──────────────────────┐          (CAT7 Kabel)          ┌──────────────────────┐
    │                      │          bis 100m              │                      │
    │  ┌────────────────┐  │                                │  ┌────────────────┐  │
    │  │ Arduino Mega   │◄─┼────────────────────────────────┤  │ Arduino Nano   │  │
    │  │ (ATmega2560)   │  │                                │  │ Every          │  │
    │  │                │  │      <SENSOR;DIST=...>         │  │ (ATmega4809)   │  │
    │  │ - Datenempfang │  │                                │  │                │  │
    │  │ - Protokoll-   │  │                                │  │ - Sensor-      │  │
    │  │   Parsing      │  │                                │  │   Messung      │  │
    │  │ - Display-     │  │                                │  │ - Daten-       │  │
    │  │   Steuerung    │  │                                │  │   Aufbereitung │  │
    │  └────────┬───────┘  │                                │  └────────┬───────┘  │
    │           │          │                                │           │          │
    │           ▼          │                                │           ▼          │
    │  ┌────────────────┐  │                                │  ┌────────────────┐  │
    │  │ Nextion        │  │                                │  │ Sensor Array   │  │
    │  │ NX4024T032     │  │                                │  │                │  │
    │  │ 3.2" Touch     │  │                                │  │ • JSN-SR04T    │  │
    │  │                │  │                                │  │ • DS18B20      │  │
    │  │ - Visualisier. │  │                                │  │ • TSW-20M      │  │
    │  │ - User Input   │  │                                │  │ • CQRSENTDS01  │  │
    │  └────────────────┘  │                                │  └────────────────┘  │
    │                      │                                │                      │
    │  ┌────────────────┐  │                                │  ┌────────────────┐  │
    │  │ 12V Netzteil   │  │                                │  │ LM2596         │  │
    │  │ (2A)           │──┼───12V über CAT7────────────────┤  │ (12V→5V)       │  │
    │  │                │  │                                │  │                │  │
    │  └────────────────┘  │                                │  └────────────────┘  │
    │                      │                                │                      │
    └──────────────────────┘                                └──────────────────────┘
         Trockene                                               IP67 wasserdicht
         Umgebung                                               Gehäuse
```

#### Systemkomponenten

**Display Node (Master):**
- Empfängt Sensordaten vom Sensor Node
- Verarbeitet und validiert empfangene Daten
- Steuert das Nextion Touchdisplay
- Bietet Benutzerinteraktion
- Zentrale Stromversorgung für beide Nodes

**Sensor Node (Slave):**
- Erfasst Messwerte von 4 Sensoren
- Verarbeitet Rohdaten lokal
- Sendet formatierte Daten periodisch
- Autonomer Betrieb ohne Bestätigung
- Powered über CAT7 vom Display Node

**Kommunikations-Link:**
- RS-485 differenzielle Übertragung
- CAT7 Ethernet-Kabel als physisches Medium
- Distanzen bis 100m möglich
- Separate Adern für Power und Daten

### 1.3 Hauptmerkmale

#### Hardware-Merkmale

- **Dual-Mikrocontroller-Architektur:** Getrennte Verarbeitung für Sensordaten und Display
- **Robuste Kommunikation:** RS-485 mit hoher Störsicherheit
- **Redundante Stromversorgung:** Parallele Adernführung im CAT7-Kabel
- **Wasserdichtes Design:** IP67-Schutz für Sensor Node
- **Kapazitives Touchdisplay:** Moderne Benutzeroberfläche

#### Software-Merkmale

- **Modulares Design:** Klare Trennung der Funktionsmodule
- **Fehlertoleranz:** Graceful Degradation bei Sensorausfällen
- **Echtzeit-Verarbeitung:** Deterministisches Timing
- **Protokoll-basiert:** Strukturierte Datenkommunikation
- **Erweiterbar:** Einfache Integration neuer Sensoren

#### Betriebsmerkmale

- **Automatischer Betrieb:** Keine manuelle Konfiguration erforderlich
- **Niedrige Latenz:** <500ms vom Sensor bis Display
- **Hohe Update-Rate:** Neue Werte alle 5 Sekunden
- **Wartungsfreundlich:** Modularer Austausch von Komponenten
- **Energieeffizient:** <2W Gesamtverbrauch

---

## 2. Technische Architektur

### 2.1 Systemarchitektur-Diagramm (Detailliert)

```
╔════════════════════════════════════════════════════════════════════════════════════╗
║                    BIOSYNC DETAILLIERTE SYSTEMARCHITEKTUR                          ║
╚════════════════════════════════════════════════════════════════════════════════════╝

┌─────────────────────── DISPLAY NODE (Master) ───────────────────────────────────┐
│                                                                                   │
│  ┌─────────────────────────────────────────────────────────────────────────┐    │
│  │                    Arduino Mega 2560 (ATmega2560)                        │    │
│  │                                                                           │    │
│  │  ┌──────────────┐  ┌──────────────┐  ┌──────────────┐  ┌──────────────┐│    │
│  │  │   USB/Debug  │  │   RS-485     │  │   Nextion    │  │  GPIO/Ext.   ││    │
│  │  │   Serial0    │  │   Serial1    │  │   Serial2    │  │   Expansion  ││    │
│  │  │   (115200)   │  │   (9600)     │  │   (9600)     │  │              ││    │
│  │  └──────┬───────┘  └──────┬───────┘  └──────┬───────┘  └──────────────┘│    │
│  │         │                 │                  │                          │    │
│  │         ▼                 ▼                  ▼                          │    │
│  │  ┌──────────────────────────────────────────────────────────────┐      │    │
│  │  │              Hauptprogramm-Loop (loop())                     │      │    │
│  │  │                                                               │      │    │
│  │  │  ┌─────────────────┐  ┌─────────────────┐  ┌──────────────┐│      │    │
│  │  │  │ RS-485 RX       │  │  Message Parser │  │  Nextion     ││      │    │
│  │  │  │ Buffer Manager  │→ │  (Protocol)     │→ │  Updater     ││      │    │
│  │  │  │                 │  │                 │  │              ││      │    │
│  │  │  │ - Ring Buffer   │  │ - Extract Data  │  │ - Format Cmd ││      │    │
│  │  │  │ - Line Detection│  │ - Validate      │  │ - Send 3xFF  ││      │    │
│  │  │  └─────────────────┘  └─────────────────┘  └──────────────┘│      │    │
│  │  │                                                               │      │    │
│  │  └───────────────────────────────────────────────────────────────┘      │    │
│  │                                                                           │    │
│  │  Memory Usage:                                                            │    │
│  │  - Flash: ~15 KB / 256 KB                                                │    │
│  │  - SRAM:  ~2 KB / 8 KB                                                   │    │
│  │                                                                           │    │
│  └───────────────────────────────────────────────────────────────────────────┘    │
│                                                                                   │
│  ┌───────────────────────────────────────────────────────────────────────────┐    │
│  │                        RS-485 Transceiver Module                          │    │
│  │                                                                            │    │
│  │  ┌──────────────────┐         ┌──────────────────┐                       │    │
│  │  │  MAX485 / SN75   │         │  Termination     │                       │    │
│  │  │  - RO → RX1      │         │  (Optional)      │                       │    │
│  │  │  - DI → TX1      │         │  120Ω A-B        │                       │    │
│  │  │  - DE/RE → GND   │         │  (>50m cable)    │                       │    │
│  │  └──────────────────┘         └──────────────────┘                       │    │
│  │         A │ B                                                             │    │
│  └───────────┼─────────────────────────────────────────────────────────────┘    │
│              │                                                                    │
│  ┌───────────┼───────────────────────────────────────────────────────────────┐  │
│  │           │          Nextion NX4024T032 Display                           │  │
│  │           │                                                                │  │
│  │  ┌────────┼──────────┐  ┌────────────────┐  ┌──────────────────────┐    │  │
│  │  │ STM32  │Controller│  │ 3.2" TFT LCD   │  │ Resistive Touch      │    │  │
│  │  │        │          │  │ 320x240 px     │  │ Controller           │    │  │
│  │  │ - RX ← TX2       │  │ 65K Colors     │  │                      │    │  │
│  │  │ - TX → RX2       │  │                │  │ Touch Events → STM32 │    │  │
│  │  └──────────────────┘  └────────────────┘  └──────────────────────┘    │  │
│  │                                                                          │  │
│  │  HMI Elements:                                                           │  │
│  │  - tDist (Text): Füllstand in cm                                         │  │
│  │  - tTemp (Text): Temperatur in °C                                        │  │
│  │  - tTurb (Text): Trübung (0-1023)                                        │  │
│  │  - tTDS (Text): TDS-Wert in ppm                                          │  │
│  │  - Timer: Screensaver (60s idle)                                         │  │
│  └──────────────────────────────────────────────────────────────────────────┘  │
│                                                                                   │
│  ┌──────────────────────────────────────────────────────────────────────────┐   │
│  │                   Stromversorgung Display Node                            │   │
│  │                                                                            │   │
│  │  ┌─────────────┐      ┌───────────────┐        ┌──────────────────┐     │   │
│  │  │ 12V/2A      │      │ LM2596        │        │ Distribution     │     │   │
│  │  │ AC/DC PSU   │─────→│ Buck Conv.    │───────→│ Mega: 5V         │     │   │
│  │  │             │  12V │ 12V→5V        │  5V    │ Nextion: 5V      │     │   │
│  │  │ - OVP       │      │ - Iout: 3A    │        │ RS-485: 5V       │     │   │
│  │  │ - OCP       │      │ - Eff.: 92%   │        │                  │     │   │
│  │  └─────────────┘      └───────────────┘        └──────────────────┘     │   │
│  │        │                                                                  │   │
│  │        └──────────── 12V über CAT7 zu Sensor Node ─────────────────────→│   │
│  └──────────────────────────────────────────────────────────────────────────┘   │
└───────────────────────────────────────────────────────────────────────────────────┘
                                          │
                                          │ CAT7 Kabel
                                          │ - Adern 1+2: 12V+
                                          │ - Adern 3+4: GND
                                          │ - Adern 5+6: RS-485 (A/B)
                                          │ - Adern 7+8: Reserve
                                          │
                                          ▼
┌─────────────────────── SENSOR NODE (Slave) ─────────────────────────────────────┐
│                                                                                   │
│  ┌──────────────────────────────────────────────────────────────────────────┐   │
│  │                   Stromversorgung Sensor Node                             │   │
│  │                                                                            │   │
│  │  ┌───────────────┐      ┌───────────────┐        ┌──────────────────┐   │   │
│  │  │ 12V vom Haus  │      │ LM2596        │        │ Distribution     │   │   │
│  │  │ (CAT7 1+2)    │─────→│ Buck Conv.    │───────→│ Nano: 5V         │   │   │
│  │  │               │  12V │ 12V→5V        │  5V    │ Sensoren: 5V     │   │   │
│  │  │               │      │ - Iout: 3A    │        │ RS-485: 5V       │   │   │
│  │  │               │      │ - Eff.: 92%   │        │                  │   │   │
│  │  └───────────────┘      └───────────────┘        └──────────────────┘   │   │
│  └──────────────────────────────────────────────────────────────────────────┘   │
│                                                                                   │
│  ┌───────────────────────────────────────────────────────────────────────────┐  │
│  │              Arduino Nano Every (ATmega4809)                               │  │
│  │                                                                             │  │
│  │  ┌──────────────┐  ┌──────────────┐  ┌──────────────┐  ┌──────────────┐ │  │
│  │  │   USB/Debug  │  │   Digital    │  │   Analog     │  │  Software    │ │  │
│  │  │   Serial     │  │   I/O        │  │   Inputs     │  │  Serial      │ │  │
│  │  │   (115200)   │  │              │  │              │  │  (9600)      │ │  │
│  │  └──────┬───────┘  └──────┬───────┘  └──────┬───────┘  └──────┬───────┘ │  │
│  │         │                 │                  │                 │         │  │
│  │         ▼                 ▼                  ▼                 ▼         │  │
│  │  ┌────────────────────────────────────────────────────────────────────┐ │  │
│  │  │              Hauptprogramm-Loop (loop())                           │ │  │
│  │  │                                                                     │ │  │
│  │  │  ┌──────────────┐  ┌──────────────┐  ┌──────────────────────────┐ │ │  │
│  │  │  │ Sensor Read  │  │ Data Process │  │  RS-485 Transmitter      │ │ │  │
│  │  │  │ Manager      │→ │ & Format     │→ │                          │ │ │  │
│  │  │  │              │  │              │  │  - Set DE HIGH           │ │ │  │
│  │  │  │ Every 5000ms │  │ Build String │  │  - Send Message          │ │ │  │
│  │  │  │              │  │              │  │  - Set DE LOW            │ │ │  │
│  │  │  └──────────────┘  └──────────────┘  └──────────────────────────┘ │ │  │
│  │  │                                                                     │ │  │
│  │  └─────────────────────────────────────────────────────────────────────┘ │  │
│  │                                                                             │  │
│  │  Memory Usage:                                                              │  │
│  │  - Flash: ~10 KB / 48 KB                                                   │  │
│  │  - SRAM:  ~1 KB / 6 KB                                                     │  │
│  │                                                                             │  │
│  └─────────────────────────────────────────────────────────────────────────────┘  │
│                                                                                   │
│  ┌───────────────────────────────────────────────────────────────────────────┐  │
│  │                        Sensor Array                                        │  │
│  │                                                                             │  │
│  │  ┌────────────────────┐  ┌────────────────────┐  ┌────────────────────┐  │  │
│  │  │ JSN-SR04T          │  │ DS18B20 (MICREEN)  │  │ TSW-20M            │  │  │
│  │  │ Ultrasonic Distance│  │ Digital Temperature│  │ Turbidity (Analog) │  │  │
│  │  │                    │  │                     │  │                    │  │  │
│  │  │ TRIG → D2          │  │ DATA → D4          │  │ OUT → A0           │  │  │
│  │  │ ECHO → D3          │  │ + 4.7kΩ Pull-up    │  │                    │  │  │
│  │  │ Range: 2-400cm     │  │ Range: -55 to 125°C│  │ Range: 0-1023      │  │  │
│  │  │ Accuracy: ±1cm     │  │ Accuracy: ±0.5°C   │  │ Response: <2s      │  │  │
│  │  │ Waterproof: IP68   │  │ Waterproof: IP68   │  │ Optical Sensor     │  │  │
│  │  └────────────────────┘  └────────────────────┘  └────────────────────┘  │  │
│  │                                                                             │  │
│  │  ┌────────────────────┐                                                    │  │
│  │  │ CQRSENTDS01        │                                                    │  │
│  │  │ TDS (Analog)       │                                                    │  │
│  │  │                    │                                                    │  │
│  │  │ OUT → A1           │                                                    │  │
│  │  │ Range: 0-1000ppm   │                                                    │  │
│  │  │ Response: <10s     │                                                    │  │
│  │  │ Electrode-based    │                                                    │  │
│  │  └────────────────────┘                                                    │  │
│  └─────────────────────────────────────────────────────────────────────────────┘  │
│                                                                                   │
│  ┌───────────────────────────────────────────────────────────────────────────┐  │
│  │                        RS-485 Transceiver Module                           │  │
│  │                                                                             │  │
│  │  ┌──────────────────┐         ┌──────────────────┐                        │  │
│  │  │  MAX485 / SN75   │         │  Termination     │                        │  │
│  │  │  - RO → D7 (RX)  │         │  (Optional)      │                        │  │
│  │  │  - DI → D6 (TX)  │         │  120Ω A-B        │                        │  │
│  │  │  - DE/RE → D5    │         │  (>50m cable)    │                        │  │
│  │  └──────────────────┘         └──────────────────┘                        │  │
│  │         A │ B                                                              │  │
│  └───────────┼────────────────────────────────────────────────────────────────┘  │
│              │                                                                    │
│              └───────────── CAT7 Adern 5+6 zurück zu Display Node ──────────────┘
│                                                                                   │
│  ┌──────────────────────────────────────────────────────────────────────────┐   │
│  │                      IP67 Enclosure                                       │   │
│  │                                                                            │   │
│  │  - Material: ABS/Polycarbonat                                             │   │
│  │  - Schutzklasse: IP67 (Staub- und Wasserdicht)                            │   │
│  │  - Cable Glands: M12 für CAT7 und Sensorkabel                             │   │
│  │  - Desiccant: Silica Gel Beutel gegen Kondensation                        │   │
│  │  - Betriebstemperatur: -10°C bis +50°C                                    │   │
│  │  - Abmessungen: ca. 150x100x70mm                                           │   │
│  └──────────────────────────────────────────────────────────────────────────┘   │
└───────────────────────────────────────────────────────────────────────────────────┘
```

### 2.2 Datenfluss-Diagramm

```
╔═══════════════════════════════════════════════════════════════════════════════╗
║                          DATENFLUSS IM BIOSYNC-SYSTEM                         ║
╚═══════════════════════════════════════════════════════════════════════════════╝

SENSOR NODE                                              DISPLAY NODE
─────────────                                            ─────────────

┌─────────────┐
│   Timer     │
│ (5 Sekunden)│
└──────┬──────┘
       │ Trigger
       ▼
┌─────────────────────────────────────────────┐
│        Sensor Measurement Cycle             │
│                                              │
│  1. measureDistance()                        │
│     ├─ Trigger Pulse (10μs)                 │
│     ├─ Wait for Echo                        │
│     ├─ Calculate Distance                   │
│     └─ Return: float (cm)                   │
│                                              │
│  2. readTemperature()                        │
│     ├─ Request Temperature (1-Wire)         │
│     ├─ Wait 750ms                           │
│     ├─ Read Value                           │
│     └─ Return: float (°C)                   │
│                                              │
│  3. readTurbidity()                          │
│     ├─ analogRead(A0)                       │
│     ├─ 10-bit ADC                           │
│     └─ Return: int (0-1023)                 │
│                                              │
│  4. readTDS()                                │
│     ├─ analogRead(A1)                       │
│     ├─ 10-bit ADC                           │
│     └─ Return: int (0-1023)                 │
│                                              │
└──────────────────┬──────────────────────────┘
                   │
                   ▼
┌─────────────────────────────────────────────┐
│        Data Formatting                      │
│                                              │
│  String msg = "<SENSOR;";                   │
│  msg += "DIST=" + String(distance, 1) + ";│
│  msg += "TMP=" + String(temp, 1) + ";";    │
│  msg += "TUR=" + String(turb) + ";";       │
│  msg += "TDS=" + String(tds) + ">\n";      │
│                                              │
│  Beispiel:                                   │
│  <SENSOR;DIST=125.3;TMP=18.2;TUR=450;       │
│   TDS=320>\n                                 │
│                                              │
└──────────────────┬──────────────────────────┘
                   │
                   ▼
┌─────────────────────────────────────────────┐
│        RS-485 Transmission                  │
│                                              │
│  1. digitalWrite(DE, HIGH)  // TX Mode      │
│  2. delay(2ms)              // Settle       │
│  3. rs485.print(msg)        // Send         │
│  4. rs485.flush()           // Wait TX      │
│  5. delay(2ms)              // Settle       │
│  6. digitalWrite(DE, LOW)   // RX Mode      │
│                                              │
└──────────────────┬──────────────────────────┘
                   │
                   │ RS-485 A/B Lines
                   │ (Differential Signal)
                   │ CAT7 Adern 5+6
                   │
                   ▼
                                            ┌─────────────────────────────┐
                                            │   RS-485 Reception          │
                                            │                              │
                                            │  Serial1 RX Interrupt       │
                                            │  └─ Byte empfangen          │
                                            │  └─ In Buffer schreiben     │
                                            │                              │
                                            └──────────────┬──────────────┘
                                                           │
                                                           ▼
                                            ┌─────────────────────────────┐
                                            │   Line Detection            │
                                            │                              │
                                            │  while (Serial1.available())│
                                            │    char c = read()          │
                                            │    if (c == '\n')           │
                                            │      process line           │
                                            │                              │
                                            └──────────────┬──────────────┘
                                                           │
                                                           ▼
                                            ┌─────────────────────────────┐
                                            │   Protocol Validation       │
                                            │                              │
                                            │  if (line.startsWith("<"))  │
                                            │  if (line.indexOf("SENSOR"))│
                                            │    Valid → Parse            │
                                            │  else                       │
                                            │    Discard                  │
                                            │                              │
                                            └──────────────┬──────────────┘
                                                           │
                                                           ▼
                                            ┌─────────────────────────────┐
                                            │   Data Parsing              │
                                            │                              │
                                            │  extract("DIST") → "125.3"  │
                                            │  extract("TMP")  → "18.2"   │
                                            │  extract("TUR")  → "450"    │
                                            │  extract("TDS")  → "320"    │
                                            │                              │
                                            │  Append units:              │
                                            │  distance = "125.3 cm"      │
                                            │  temperature = "18.2 °C"    │
                                            │  turbidity = "450"          │
                                            │  tds = "320"                │
                                            │                              │
                                            └──────────────┬──────────────┘
                                                           │
                                                           ▼
                                            ┌─────────────────────────────┐
                                            │   Nextion Update            │
                                            │                              │
                                            │  Für jedes Feld:            │
                                            │                              │
                                            │  1. Format Command          │
                                            │     "tDist.txt=\"125.3 cm\"│
                                            │                              │
                                            │  2. Send to Nextion         │
                                            │     Serial2.print(cmd)      │
                                            │     Serial2.write(0xFF)     │
                                            │     Serial2.write(0xFF)     │
                                            │     Serial2.write(0xFF)     │
                                            │                              │
                                            │  3. Delay 10ms              │
                                            │     (Nextion processing)    │
                                            │                              │
                                            └──────────────┬──────────────┘
                                                           │
                                                           ▼
                                            ┌─────────────────────────────┐
                                            │   Display Rendering         │
                                            │                              │
                                            │  Nextion STM32:             │
                                            │  - Parse Command            │
                                            │  - Update Text Object       │
                                            │  - Render to LCD            │
                                            │  - Send ACK (optional)      │
                                            │                              │
                                            └─────────────────────────────┘

TIMING:
───────
Sensor Measurement:     ~800ms (DS18B20 dominiert)
Data Formatting:        ~5ms
RS-485 Transmission:    ~10ms @ 9600 baud
RS-485 Reception:       ~10ms
Parsing:                ~2ms
Nextion Update:         ~50ms (4 Felder × 10ms + overhead)
Display Rendering:      ~20ms

TOTAL LATENCY:          ~900ms vom Sensor bis Display
UPDATE INTERVAL:        5000ms (5 Sekunden)
```

### 2.3 Hardware-Architektur beider Nodes

#### 2.3.1 Sensor Node Hardware-Block-Diagramm

```
┌──────────────────────────────────────────────────────────────────────────────┐
│                         SENSOR NODE HARDWARE ARCHITEKTUR                      │
└──────────────────────────────────────────────────────────────────────────────┘

                              ┌──────────────────────┐
                              │  ATmega4809 MCU      │
                              │  (Arduino Nano Every)│
                              │                      │
                              │  Clock: 20 MHz       │
                              │  Flash: 48 KB        │
                              │  SRAM:  6 KB         │
                              │  EEPROM: 256 B       │
                              └──────────┬───────────┘
                                         │
                ┌────────────────────────┼────────────────────────┐
                │                        │                        │
                │                        │                        │
        ┌───────▼────────┐      ┌───────▼────────┐      ┌───────▼────────┐
        │  Digital I/O    │      │  Analog Input   │      │  Serial I/O    │
        │                 │      │                 │      │                 │
        │  D2: TRIG       │      │  A0: Turbidity  │      │  D0: USB RX    │
        │  D3: ECHO       │      │  A1: TDS        │      │  D1: USB TX    │
        │  D4: OneWire    │      │                 │      │  D6: RS485 TX  │
        │  D5: RS485 DE   │      │  10-bit ADC     │      │  D7: RS485 RX  │
        │                 │      │  0-5V Range     │      │                 │
        └───────┬─────────┘      └───────┬─────────┘      └───────┬─────────┘
                │                        │                        │
                │                        │                        │
        ┌───────▼─────────┐      ┌───────▼─────────┐      ┌───────▼─────────┐
        │ JSN-SR04T       │      │ TSW-20M         │      │ MAX485          │
        │ Ultrasonic      │      │ Turbidity       │      │ RS-485 Driver   │
        │                 │      │                 │      │                 │
        │ - Waterproof    │      │ - Optical       │      │ - A Line        │
        │ - 2-400cm       │      │ - 0-5V Out      │      │ - B Line        │
        │ - VCC: 5V       │      │ - VCC: 5V       │      │ - DE/RE Ctrl    │
        │ - I: 15mA       │      │ - I: 5mA        │      │ - Half Duplex   │
        └─────────────────┘      └─────────────────┘      └────────┬────────┘
                                                                    │
        ┌────────────────────────────────────────────────┐          │
        │ DS18B20 (1-Wire)    CQRSENTDS01 (TDS)         │          │
        │ Temperature         Conductivity               │          │
        │                                                │          │
        │ - Stainless Steel   - Electrode                │          │
        │ - -55 to +125°C     - 0-1000ppm                │          │
        │ - ±0.5°C Accuracy   - 0-5V Out                 │          │
        │ - 4.7kΩ Pull-up     - VCC: 5V                  │          │
        │ - VCC: 5V           - I: 5mA                   │          │
        │ - I: 1mA            │                          │          │
        └────────────────────────────────────────────────┘          │
                                                                    │
                              ┌──────────────┐                      │
                              │  LM2596      │                      │
                              │  Buck Conv.  │                      │
                              │              │                      │
                              │  Vin: 12V    │                      │
                              │  Vout: 5V    │                      │
                              │  Iout: 3A    │                      │
                              │  Eff: 92%    │                      │
                              └───┬──────┬───┘                      │
                                  │      │                          │
                    12V+ ─────────┘      └────── 5V+ ──────────────┬┘
                    (CAT7 1+2)                                      │
                                                           To all 5V devices
                    GND ────────────────────── GND ──────────────────┘
                    (CAT7 3+4)

POWER CONSUMPTION:
─────────────────
ATmega4809:      30 mA @ 5V, 20 MHz
JSN-SR04T:       15 mA @ 5V (measurement)
DS18B20:          1 mA @ 5V (conversion)
TSW-20M:          5 mA @ 5V (continuous)
CQRSENTDS01:      5 mA @ 5V (continuous)
MAX485:           1 mA @ 5V (receive mode)
                  5 mA @ 5V (transmit mode, <1% duty)

TOTAL AVG:       ~60 mA @ 5V = 0.3W @ 5V
               = ~30 mA @ 12V (considering converter efficiency)

PEAK (TX):       ~90 mA @ 5V = 0.45W @ 5V

THERMAL:
────────
LM2596 at 30mA input, 60mA output:
Power Loss = (12V × 0.03A) - (5V × 0.06A) = 0.36W - 0.30W = 0.06W
Temperature Rise: ~2-3°C above ambient (with heatsink)
```

#### 2.3.2 Display Node Hardware-Block-Diagramm

```
┌──────────────────────────────────────────────────────────────────────────────┐
│                        DISPLAY NODE HARDWARE ARCHITEKTUR                      │
└──────────────────────────────────────────────────────────────────────────────┘

                              ┌──────────────────────┐
                              │  ATmega2560 MCU      │
                              │  (Arduino Mega 2560) │
                              │                      │
                              │  Clock: 16 MHz       │
                              │  Flash: 256 KB       │
                              │  SRAM:  8 KB         │
                              │  EEPROM: 4 KB        │
                              └──────────┬───────────┘
                                         │
                ┌────────────────────────┼────────────────────────┐
                │                        │                        │
                │                        │                        │
        ┌───────▼────────┐      ┌───────▼────────┐      ┌───────▼────────┐
        │  Serial 0       │      │  Serial 1       │      │  Serial 2       │
        │  (USB)          │      │  (RS-485)       │      │  (Nextion)      │
        │                 │      │                 │      │                 │
        │  D0: RX         │      │  D19: RX1       │      │  D17: RX2       │
        │  D1: TX         │      │  D18: TX1       │      │  D16: TX2       │
        │  115200 baud    │      │  9600 baud      │      │  9600 baud      │
        │  Debug Only     │      │  Data Reception │      │  Display Ctrl   │
        └───────┬─────────┘      └───────┬─────────┘      └───────┬─────────┘
                │                        │                        │
                │                        │                        │
        Not used in         ┌────────────▼─────────┐     ┌───────▼─────────┐
        deployed system     │ MAX485               │     │ Nextion Display │
                            │ RS-485 Receiver      │     │ NX4024T032      │
                            │                      │     │                 │
                            │ - A Line ← CAT7 5    │     │ STM32F030       │
                            │ - B Line ← CAT7 6    │     │ 3.2" 320x240    │
                            │ - DE/RE → GND        │     │ 65K Colors      │
                            │   (RX only mode)     │     │ Resistive Touch │
                            │                      │     │                 │
                            │ - VCC: 5V            │     │ - VCC: 5V       │
                            │ - I: 1mA (RX)        │     │ - I: 85mA       │
                            └──────────────────────┘     └─────────────────┘

                              ┌──────────────────────┐
                              │  12V AC/DC Power     │
                              │  Supply              │
                              │                      │
                              │  Input: 100-240V AC  │
                              │  Output: 12V/2A      │
                              │  Protection:         │
                              │  - OVP, OCP, SCP     │
                              └───────────┬──────────┘
                                          │
                                          │ 12V
                                          ▼
                              ┌──────────────────────┐
                              │  LM2596 Buck Conv.   │
                              │                      │
                              │  Vin: 12V            │
                              │  Vout: 5V (adjusted) │
                              │  Iout: 3A max        │
                              │  Efficiency: 92%     │
                              └───┬──────────────┬───┘
                                  │              │
                    5V+ ──────────┴──────┬───────┴──── To all 5V devices
                                         │
                                         ├─ ATmega2560: 50mA
                                         ├─ MAX485: 1mA
                                         └─ Nextion: 85mA
                    GND ─────────────────┴─────────────

                    12V+ ────────────────────────────── To Sensor Node
                    (CAT7 1+2)                          (CAT7 1+2)

                    GND ─────────────────────────────── To Sensor Node
                    (CAT7 3+4)                          (CAT7 3+4)

POWER CONSUMPTION:
─────────────────
ATmega2560:      50 mA @ 5V = 0.25W
MAX485 (RX):      1 mA @ 5V = 0.005W
Nextion Display: 85 mA @ 5V = 0.425W

TOTAL Display Node:  ~136 mA @ 5V = 0.68W @ 5V

12V Power Supply Output:
- Display Node @ 5V:  0.68W / 0.92 (eff.) = 0.74W @ 12V = 62mA
- Sensor Node @ 12V:  0.30W / 1 (direct)   = 0.30W @ 12V = 25mA
- TOTAL 12V:          1.04W @ 12V ≈ 87mA

12V/2A PSU is oversized → Good for:
- Future expansions
- Inrush current handling
- Long-term reliability

THERMAL:
────────
LM2596 at 87mA input, 136mA output @ 5V:
Power Loss = (12V × 0.087A) - (5V × 0.136A) = 1.04W - 0.68W = 0.36W
Temperature Rise: ~10-15°C above ambient (with heatsink)

12V PSU at ~90mA load (rated 2A):
Load: 4.5% of rated capacity
Efficiency: ~85% (light load)
Temperature Rise: <20°C above ambient
```


### 2.4 Software-Architektur und Modulstruktur

#### 2.4.1 Sensor Node Software-Architektur

```
╔════════════════════════════════════════════════════════════════════════════╗
║              SENSOR NODE SOFTWARE ARCHITEKTUR                              ║
╚════════════════════════════════════════════════════════════════════════════╝

┌──────────────────────────────────────────────────────────────────────────┐
│                            SensorNode.ino                                 │
│                          (Hauptprogramm)                                  │
└────────────────────────────┬─────────────────────────────────────────────┘
                             │
         ┌───────────────────┼───────────────────┐
         │                   │                   │
         ▼                   ▼                   ▼
┌──────────────┐    ┌──────────────┐    ┌──────────────┐
│  config.h    │    │  sensors.h   │    │  rs485.h     │
│              │    │  sensors.cpp │    │  rs485.cpp   │
│              │    │              │    │              │
│ - Pin Defs   │    │ - Init       │    │ - Init       │
│ - Constants  │    │ - Distance   │    │ - Send       │
│ - Timing     │    │ - Temp       │    │ - DE Ctrl    │
│              │    │ - Turbidity  │    │              │
│              │    │ - TDS        │    │              │
└──────────────┘    └──────┬───────┘    └──────┬───────┘
                           │                   │
                           │                   │
              ┌────────────┴───────────────────┴────────────┐
              │         External Libraries                   │
              │                                              │
              │  ┌────────────────┐  ┌───────────────────┐  │
              │  │ OneWire.h      │  │ SoftwareSerial.h  │  │
              │  │ (1-Wire Bus)   │  │ (RS-485 Serial)   │  │
              │  └────────────────┘  └───────────────────┘  │
              │  ┌────────────────┐                         │
              │  │Dallas          │                         │
              │  │Temperature.h   │                         │
              │  │(DS18B20 Driver)│                         │
              │  └────────────────┘                         │
              └──────────────────────────────────────────────┘

PROGRAM FLOW:
─────────────

setup():
  ├─ Serial.begin(115200)           // Debug-Ausgabe
  ├─ sensors_begin()                // Sensoren initialisieren
  │   ├─ pinMode(TRIG_PIN, OUTPUT)
  │   ├─ pinMode(ECHO_PIN, INPUT)
  │   ├─ digitalWrite(TRIG_PIN, LOW)
  │   ├─ dsSensor.begin()           // OneWire DS18B20
  │   └─ delay(50)
  └─ rs485_begin()                  // RS-485 initialisieren
      ├─ pinMode(RS485_DE, OUTPUT)
      ├─ digitalWrite(RS485_DE, LOW)
      └─ rs485.begin(9600)

loop():
  ├─ Check Timer: millis() - lastSend >= SEND_INTERVAL (5000ms)
  │
  ├─ If Timer Expired:
  │   ├─ lastSend = millis()
  │   │
  │   ├─ measureDistance()          // JSN-SR04T
  │   │   ├─ Trigger Pulse
  │   │   ├─ pulseIn(ECHO_PIN)
  │   │   ├─ Calculate: duration / 58.0
  │   │   └─ Return: float (cm)
  │   │
  │   ├─ readTemperature()          // DS18B20
  │   │   ├─ dsSensor.requestTemperatures()
  │   │   ├─ dsSensor.getTempCByIndex(0)
  │   │   ├─ Check DEVICE_DISCONNECTED_C
  │   │   └─ Return: float (°C)
  │   │
  │   ├─ readTurbidity()            // TSW-20M
  │   │   ├─ analogRead(TURB_PIN)
  │   │   └─ Return: int (0-1023)
  │   │
  │   ├─ readTDS()                  // CQRSENTDS01
  │   │   ├─ analogRead(TDS_PIN)
  │   │   └─ Return: int (0-1023)
  │   │
  │   ├─ Format Message String
  │   │   ├─ "<SENSOR;"
  │   │   ├─ "DIST=" + distance + ";"
  │   │   ├─ "TMP=" + temperature + ";"
  │   │   ├─ "TUR=" + turbidity + ";"
  │   │   ├─ "TDS=" + tds + ">"
  │   │   └─ "\n"
  │   │
  │   ├─ rs485_send(msg)            // RS-485 Transmission
  │   │   ├─ digitalWrite(RS485_DE, HIGH)
  │   │   ├─ delay(2)
  │   │   ├─ rs485.print(msg)
  │   │   ├─ rs485.flush()
  │   │   ├─ delay(2)
  │   │   └─ digitalWrite(RS485_DE, LOW)
  │   │
  │   └─ Serial.print("Gesendet: " + msg)  // Debug
  │
  └─ Continue (no blocking)

MEMORY LAYOUT:
──────────────
Flash Usage:    ~10 KB / 48 KB (21%)
  - Arduino Core:        ~6 KB
  - OneWire Library:     ~1 KB
  - DallasTemp Library:  ~1 KB
  - SoftwareSerial:      ~0.5 KB
  - Application Code:    ~1.5 KB

SRAM Usage:     ~1 KB / 6 KB (17%)
  - Global Variables:    ~200 B
  - OneWire Objects:     ~100 B
  - String Buffers:      ~500 B
  - Stack (runtime):     ~200 B

EEPROM Usage:   0 B / 256 B (unused, available for calibration data)
```

#### 2.4.2 Display Node Software-Architektur

```
╔════════════════════════════════════════════════════════════════════════════╗
║              DISPLAY NODE SOFTWARE ARCHITEKTUR                             ║
╚════════════════════════════════════════════════════════════════════════════╝

┌──────────────────────────────────────────────────────────────────────────┐
│                            DisplayNode.ino                                │
│                          (Hauptprogramm)                                  │
└────────────────────────────┬─────────────────────────────────────────────┘
                             │
         ┌───────────────────┼───────────────────┬─────────────────┐
         │                   │                   │                 │
         ▼                   ▼                   ▼                 ▼
┌──────────────┐    ┌──────────────┐    ┌──────────────┐  ┌──────────────┐
│  config.h    │    │  rs485.h     │    │  nextion.h   │  │  parser.h    │
│              │    │  rs485.cpp   │    │  nextion.cpp │  │  parser.cpp  │
│              │    │              │    │              │  │              │
│ - Serials    │    │ - Init       │    │ - Init       │  │ - Extract    │
│ - Pin Defs   │    │ - Receive    │    │ - Update     │  │ - Validate   │
│ - HMI Names  │    │              │    │ - Send Cmd   │  │ - Parse Msg  │
│              │    │              │    │ - 3xFF End   │  │              │
└──────────────┘    └──────────────┘    └──────────────┘  └──────────────┘

PROGRAM FLOW:
─────────────

setup():
  ├─ Serial.begin(115200)           // Debug-Ausgabe
  ├─ rs485_begin()                  // RS-485 Empfang initialisieren
  │   ├─ pinMode(RS485_DE_PIN, OUTPUT) [optional]
  │   ├─ digitalWrite(RS485_DE_PIN, LOW)
  │   └─ Serial1.begin(9600)
  └─ nextion_begin()                // Nextion initialisieren
      └─ Serial2.begin(9600)

loop():
  ├─ while (Serial1.available())    // RS-485 Buffer prüfen
  │   ├─ char c = Serial1.read()
  │   │
  │   ├─ if (c == '\n')             // Zeilenende erkannt
  │   │   ├─ line = rs485Buffer
  │   │   ├─ rs485Buffer = ""
  │   │   ├─ line.trim()
  │   │   │
  │   │   ├─ if (line.startsWith("<") && line.indexOf("SENSOR") >= 0)
  │   │   │   │
  │   │   │   ├─ parseSensorMessage(line)
  │   │   │   │   ├─ extract("DIST") → "125.3"
  │   │   │   │   ├─ extract("TMP")  → "18.2"
  │   │   │   │   ├─ extract("TUR")  → "450"
  │   │   │   │   ├─ extract("TDS")  → "320"
  │   │   │   │   ├─ Append Units
  │   │   │   │   └─ Return: SensorData struct
  │   │   │   │
  │   │   │   └─ nextion_update(SensorData)
  │   │   │       ├─ sendToNextion("tDist.txt=\"125.3 cm\"")
  │   │   │       │   ├─ Serial2.print(cmd)
  │   │   │       │   ├─ Serial2.write(0xFF)
  │   │   │       │   ├─ Serial2.write(0xFF)
  │   │   │       │   └─ Serial2.write(0xFF)
  │   │   │       ├─ delay(10)
  │   │   │       ├─ sendToNextion("tTemp.txt=\"18.2 °C\"")
  │   │   │       ├─ delay(10)
  │   │   │       ├─ sendToNextion("tTurb.txt=\"450\"")
  │   │   │       ├─ delay(10)
  │   │   │       └─ sendToNextion("tTDS.txt=\"320\"")
  │   │   │
  │   │   └─ Serial.println("Empfangen: " + line)  // Debug
  │   │
  │   ├─ else
  │   │   ├─ rs485Buffer += c
  │   │   └─ if (rs485Buffer.length() > 1024)  // Overflow-Schutz
  │   │       └─ rs485Buffer = ""
  │
  └─ Continue (non-blocking)

MEMORY LAYOUT:
──────────────
Flash Usage:    ~15 KB / 256 KB (6%)
  - Arduino Core:        ~8 KB
  - Application Code:    ~7 KB
  - Parser Logic:        ~2 KB
  - Nextion Interface:   ~2 KB
  - RS-485 Handler:      ~1 KB

SRAM Usage:     ~2 KB / 8 KB (25%)
  - rs485Buffer (String):    ~1 KB (dynamic)
  - SensorData struct:       ~100 B
  - Nextion cmd buffers:     ~500 B
  - Stack (runtime):         ~400 B

DATA STRUCTURES:
────────────────
struct SensorData {
  String distance;      // "125.3 cm"
  String temperature;   // "18.2 °C"
  String turbidity;     // "450"
  String tds;           // "320"
};
```

---

## 3. Systemkomponenten

### 3.1 Mikrocontroller

#### 3.1.1 Arduino Nano Every (Sensor Node)

**Basierte auf ATmega4809 Mikrocontroller**

**Technische Spezifikationen:**

| Parameter | Wert | Notizen |
|-----------|------|---------|
| Architektur | AVR (8-bit RISC) | Verbessert gegenüber ATmega328P |
| CPU Takt | 20 MHz | Höher als Nano (16 MHz) |
| Flash Memory | 48 KB | 3x größer als Nano |
| SRAM | 6 KB | 3x größer als Nano |
| EEPROM | 256 Bytes | Für Kalibrierungsdaten |
| Betriebsspannung | 5V | Über VIN (7-12V) oder 5V-Pin |
| I/O Pins | 22 Digital, 8 Analog | 14 davon PWM-fähig |
| ADC Auflösung | 10-bit (0-1023) | 0-5V Bereich |
| UART | 1x Hardware | Für USB (Debug) |
| I2C | 1x | Nicht verwendet |
| SPI | 1x | Nicht verwendet |
| Stromverbrauch | 30 mA @ 5V, 20 MHz | Active Mode |
| Maße | 45 × 18 mm | Kompatibel mit Breadboards |

**Pin-Nutzung im BioSync-System:**

| Pin | Funktion | Komponente | Signal-Typ | Notizen |
|-----|----------|------------|------------|---------|
| VIN | 5V Eingang | LM2596 OUT+ | Power | Von Step-Down-Wandler |
| GND | Ground | Gemeinsam | Power | Alle Komponenten verbunden |
| D2 | Trigger | JSN-SR04T | Digital Out | 10μs Puls |
| D3 | Echo | JSN-SR04T | Digital In | Pulslänge = Distanz |
| D4 | 1-Wire Data | DS18B20 | Digital I/O | Mit 4.7kΩ Pull-up |
| D5 | DE/RE | RS-485 Modul | Digital Out | TX-Kontrolle |
| D6 | TX | RS-485 Modul (DI) | Serial TX | SoftwareSerial |
| D7 | RX | RS-485 Modul (RO) | Serial RX | SoftwareSerial |
| A0 | Trübung | TSW-20M | Analog In | 0-1023 |
| A1 | TDS | CQRSENTDS01 | Analog In | 0-1023 |
| D0 | USB RX | PC (Debug) | Serial RX | Hardware Serial |
| D1 | USB TX | PC (Debug) | Serial TX | Hardware Serial |

**Besonderheiten:**

- **Kein Bootloader-Delay:** ATmega4809 startet sofort nach Power-On
- **Erweiterte Peripherie:** Mehr Timer und Interrupts als ATmega328P
- **Lower Power Modi:** Sleep-Modi für zukünftige Batteriebetrieb-Erweiterungen
- **Built-in LED:** D13 für Status-Anzeige (nicht verwendet)

**Elektrische Eigenschaften:**

- **Absolute Maximum Ratings:**
  - VIN: -0.5V bis +20V
  - 5V Pin: 4.5V bis 5.5V
  - I/O Pin: -0.5V bis Vcc+0.5V
  - Max Current per I/O: 40 mA
  - Max Total I/O Current: 200 mA

- **Empfohlene Betriebsbedingungen:**
  - VIN: 7V bis 12V (für LDO-Regler)
  - 5V Pin: 5.0V ±5%
  - I/O Drive: 20 mA per Pin
  - Temperature: -40°C bis +85°C

#### 3.1.2 Arduino Mega 2560 (Display Node)

**Basiert auf ATmega2560 Mikrocontroller**

**Technische Spezifikationen:**

| Parameter | Wert | Notizen |
|-----------|------|---------|
| Architektur | AVR (8-bit RISC) | Großer Bruder des ATmega328P |
| CPU Takt | 16 MHz | Standard Arduino Takt |
| Flash Memory | 256 KB | Viel Platz für komplexe Anwendungen |
| SRAM | 8 KB | Genug für große Daten-Buffers |
| EEPROM | 4 KB | Für Konfiguration und Logs |
| Betriebsspannung | 5V | Über VIN (7-12V) oder 5V-Pin |
| I/O Pins | 54 Digital, 16 Analog | 15 davon PWM-fähig |
| ADC Auflösung | 10-bit (0-1023) | 0-5V Bereich |
| **UART** | **4x Hardware Serial** | **Hauptvorteil für BioSync!** |
| I2C | 1x (TWI) | Nicht verwendet |
| SPI | 1x | Nicht verwendet |
| Stromverbrauch | 50 mA @ 5V, 16 MHz | Active Mode |
| Maße | 101.5 × 53.3 mm | Groß, aber viele Pins |

**Pin-Nutzung im BioSync-System:**

| Pin | Funktion | Komponente | Signal-Typ | Notizen |
|-----|----------|------------|------------|---------|
| 5V | 5V Eingang | LM2596 OUT+ | Power | Von Step-Down-Wandler |
| GND | Ground | Gemeinsam | Power | Alle Komponenten verbunden |
| D0 | USB RX | PC (Debug) | Serial0 RX | Hardware Serial 0 |
| D1 | USB TX | PC (Debug) | Serial0 TX | Hardware Serial 0 |
| **D19** | **RS-485 RX** | **RS-485 Modul (RO)** | **Serial1 RX** | **Hardware Serial 1** |
| **D18** | **RS-485 TX** | **RS-485 Modul (DI)** | **Serial1 TX** | **Hardware Serial 1** |
| **D17** | **Nextion RX** | **Nextion Display (TX)** | **Serial2 RX** | **Hardware Serial 2** |
| **D16** | **Nextion TX** | **Nextion Display (RX)** | **Serial2 TX** | **Hardware Serial 2** |
| D10 | DE/RE | RS-485 Modul | Digital Out | Optional, meist GND |
| D2-D53 | GPIO | Expansion | Digital I/O | Reserve für Erweiterungen |
| A0-A15 | Analog In | Expansion | Analog In | Reserve für Erweiterungen |

**Hauptvorteil: 4 Hardware-Serial-Ports**

Der Arduino Mega ist speziell wegen seiner **4 Hardware-UART-Ports** gewählt:

1. **Serial0 (D0/D1):** USB-Debugging (nicht in Produktion verwendet)
2. **Serial1 (D19/D18):** RS-485 Kommunikation mit Sensor Node
3. **Serial2 (D17/D16):** Nextion Display Kommunikation
4. **Serial3 (D15/D14):** Reserve für zukünftige Erweiterungen (z.B. GPS, GSM)

**Warum Hardware-Serial statt SoftwareSerial?**

| Aspekt | Hardware Serial | Software Serial |
|--------|----------------|----------------|
| Stabilität | ⭐⭐⭐⭐⭐ Sehr stabil | ⭐⭐⭐ Okay, aber anfällig |
| Max. Baudrate | Bis 2 Mbps | Bis 115200 bps (instabil) |
| Interrupts | Keine Beeinträchtigung | Blockiert andere Interrupts |
| CPU Last | Minimal (<1%) | ~10-20% bei 9600 baud |
| Buffer | 64 Bytes Hardware-FIFO | Software-Buffer (fehleranfällig) |
| Gleichzeitigkeit | Mehrere gleichzeitig | Nur eines aktiv |

**Elektrische Eigenschaften:**

- **Absolute Maximum Ratings:**
  - VIN: -0.5V bis +20V
  - 5V Pin: 4.5V bis 5.5V
  - I/O Pin: -0.5V bis Vcc+0.5V
  - Max Current per I/O: 40 mA
  - Max Total I/O Current: 200 mA (Summe aller Pins)

- **Empfohlene Betriebsbedingungen:**
  - VIN: 7V bis 12V (für LDO-Regler on-board)
  - 5V Pin: 5.0V ±5%
  - I/O Drive: 20 mA per Pin (empfohlen)
  - Temperature: -40°C bis +85°C

### 3.2 Sensoren (Detailliert)

#### 3.2.1 JSN-SR04T - Wasserdichter Ultraschallsensor

**Funktionsprinzip: Ultraschall-Laufzeitmessung**

Der JSN-SR04T sendet einen 40 kHz Ultraschallimpuls aus und misst die Zeit bis zum Empfang des Echos. Die Distanz wird aus der Laufzeit berechnet:

```
Distanz (cm) = (Laufzeit (μs) × Schallgeschwindigkeit (cm/μs)) / 2

Bei 20°C Lufttemperatur:
Schallgeschwindigkeit ≈ 343 m/s = 0.0343 cm/μs
                       ≈ 1 cm / 29 μs (Hin- und Rückweg)
                       ≈ 1 cm / 58 μs (nur Hinweg)

Vereinfacht: Distanz (cm) = Laufzeit (μs) / 58
```

**Technische Spezifikationen:**

| Parameter | Wert | Einheit | Notizen |
|-----------|------|---------|---------|
| Betriebsspannung | 5 | V DC | ±10% toleriert |
| Stromverbrauch | 15 (Messung), <2 (Standby) | mA | Nur während Messung |
| Frequenz | 40 | kHz | Ultraschall |
| Messbereich | 25 - 450 | cm | Blind zone: <25cm |
| Auflösung | 1 | cm | Theoretisch 0.3cm |
| Genauigkeit | ±1 | cm | Bei idealen Bedingungen |
| Öffnungswinkel | 75 | ° | Kegelförmig |
| Trigger-Puls | 10 | μs | HIGH-Puls zum Starten |
| Echo-Puls | Proportional | μs | HIGH-Dauer = Distanz |
| Max. Echo-Wartezeit | 38 | ms | Timeout für max. Distanz |
| Schutzklasse | IP68 | - | Vollständig wasserdicht |
| Betriebstemperatur | -10 bis +60 | °C | Erweitert für Outdoor |
| Kabel-länge | 2.5 | m | Standard, erweiterbar |

**Pinbelegung:**

| Pin | Farbe (Standard) | Funktion | Verbindung |
|-----|------------------|----------|------------|
| 1 | Rot | VCC (+5V) | Arduino 5V |
| 2 | Braun | TRIG | Arduino D2 |
| 3 | Gelb | ECHO | Arduino D3 |
| 4 | Schwarz | GND | Arduino GND |

**Verkabelung und Anschluss:**

```
JSN-SR04T Main Board          Separate Transducer Probe
┌─────────────────┐           ┌─────────────────┐
│                 │           │    Waterproof   │
│  VCC   ────────┼───────────┤  40kHz Ultra-   │
│  TRIG  ─────┐  │ 2.5m      │  sonic Trans-   │
│  ECHO  ───┐ │  │ Cable     │  ducer & RX     │
│  GND   ─┐ │ │  │           │                 │
└────────┼┼─┼──────┘           └─────────────────┘
         ││ │                  (Submersible, IP68)
         ││ │
      To Arduino Nano Every:
      ││ │
      ││ └── D2 (TRIG)
      │└──── D3 (ECHO)
      └───── GND

VCC ─────── 5V (aus LM2596)
```

**Software-Integration:**

```cpp
// In sensors.cpp:

float measureDistance() {
  // 1. Sensor zurücksetzen
  digitalWrite(TRIG_PIN, LOW);
  delayMicroseconds(2);
  
  // 2. Trigger-Puls senden (10μs HIGH)
  digitalWrite(TRIG_PIN, HIGH);
  delayMicroseconds(10);
  digitalWrite(TRIG_PIN, LOW);
  
  // 3. Echo-Puls messen (Timeout 30ms = ~500cm max)
  unsigned long duration = pulseIn(ECHO_PIN, HIGH, 30000);
  
  // 4. Timeout-Check (Sensor außerhalb Reichweite)
  if (duration == 0) {
    return -1.0;  // Fehlercode
  }
  
  // 5. Berechnung: duration(μs) / 58 = distance(cm)
  float distance = duration / 58.0;
  
  return distance;
}
```

**Messgenauigkeit und Fehlereinflüsse:**

| Einflussfaktor | Auswirkung | Kompensation |
|----------------|------------|--------------|
| Lufttemperatur | ±1% pro 10°C | Temperaturkorrektur-Formel anwenden |
| Luftfeuchtigkeit | <0.5% | Vernachlässigbar |
| Winkel zur Oberfläche | Bei >15° ungenau | Sensor senkrecht montieren |
| Schaumbildung | Echo-Dämpfung | Wasseroberfläche bevorzugen |
| Mehrfach-Reflexionen | Geister-Echos | Software-Filter (Plausibilität) |
| Elektrische Störungen | Sporadische Fehler | Abschirmung, Fehlerfilter |

**Temperatur-Korrektur (optional):**

```cpp
// Wenn höhere Genauigkeit benötigt:
float measureDistanceCompensated(float tempCelsius) {
  unsigned long duration = pulseIn(ECHO_PIN, HIGH, 30000);
  if (duration == 0) return -1.0;
  
  // Schallgeschwindigkeit bei Temp. T:
  // v(T) = 331.3 + 0.606 × T  [m/s]
  float speedOfSound = 331.3 + 0.606 * tempCelsius;  // m/s
  
  // Distanz berechnen: d = (v × t) / 2
  // speedOfSound in cm/μs = speedOfSound / 10000
  float distance = (speedOfSound / 10000.0) * duration / 2.0;
  
  return distance;
}
```

**Kalibrierung (siehe CALIBRATION_PROCEDURES.md für Details):**

1. **Offset-Kalibrierung:**
   - Sensor auf bekannte Distanz (z.B. 100 cm) montieren
   - Mehrere Messungen durchführen (n=20)
   - Mittelwert berechnen
   - Offset = Sollwert - Mittelwert
   - Formel: `distance_korrigiert = distance_roh + offset`

2. **Skalierungsfaktor:**
   - Standard: 58 μs/cm
   - Bei Abweichungen: Faktor anpassen (z.B. 57.5 oder 58.5)
   - Über mehrere Distanzen (50cm, 100cm, 200cm) validieren

**Montage und Installation:**

- **Position:** Über der Wasseroberfläche, senkrecht nach unten
- **Abstand:** Mindestens 25 cm über höchstem Wasserstand (Blind Zone)
- **Befestigung:** Stabile Halterung, keine Vibrationen
- **Kabelführung:** Wasserdichte Kabelverschraubung, Zugentlastung
- **Ausrichtung:** Kegelwinkel 75° beachten, keine Hindernisse im Kegel

**Wartung:**

- **Reinigung:** Transducer alle 3 Monate mit destilliertem Wasser reinigen
- **Sichtprüfung:** Kabel auf Beschädigungen prüfen
- **Kalibrierung:** Jährlich Offset-Kalibrierung wiederholen
- **Austausch:** Bei Genauigkeitsverlust oder Beschädigung

**Fehlerdiagnose:**

| Symptom | Ursache | Lösung |
|---------|---------|--------|
| Konstant -1.0 (Timeout) | Echo zu schwach oder Sensor defekt | 1. Reichweite prüfen 2. Kabel prüfen 3. Sensor tauschen |
| Unstabile Werte (Jitter) | Schaumbildung oder Mehrfach-Reflexion | Software-Filter (gleitender Mittelwert) |
| Konstant 0 cm | ECHO-Pin nicht verbunden | Verkabelung prüfen (D3) |
| Werte zu hoch | Falscher Skalierungsfaktor | Faktor von 58 anpassen (z.B. 57 oder 59) |

